...
/*void imprimir_hash(hash_t* hash){
    ...
    for(size_t i=0; i<hash->capacidad; i++){
        if(hash->vector[i].ocupado  ==  true){
            printf("\n%i nodo: \n--Dato: %s \n--Clave: %s\n", i, hash->vector[i].nodo->dato, hash->vector[i].nodo->clave);
        }
    }
}*/
...
int main(){
  ...
  for(int i=0; i<10000; i++){
      hash_insertar(hash, clave, "Auto de Mariano");
      proxima_clave(clave);
  }

  hash_insertar(hash, "clave", "Auto de Mariano");
  hash_insertar(hash, "aaaaa", "Auto de Mariano");
  ...
  for(int i=0; i<3000; i++){
      ...
      proxima_clave(clave1);
  }

  printf("Ahora hay %i", hash_cantidad(hash));
